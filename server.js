...
//
// GROUND TRUTH from an evdev capture of the actual remote (do not "correct"
// this from the spec table — the spec table was wrong):
//   long-press ArrowUp emits **Shift+F5**, NOT plain F5. That is why a
//   `key="F5"` bind never matched and the page kept hard-reloading: labwc
//   treats S-F5 as a different chord, doesn't match key="F5", and forwards
//   it to chromium (Shift+F5 = hard reload). We bind BOTH the plain and the
//   Shift variant because this remote layers Shift onto its long-press codes;
//   binding both is cheap insurance and harmless for keys nothing needs.
...
function buildLabwcConfig(suppressKeys) {
  const binds = (Array.isArray(suppressKeys) ? suppressKeys : [])
    ...
    .filter(Boolean)
    .flatMap((k) => (Array.isArray(k.keybind) ? k.keybind : [k.keybind]))
    .map((kb) => `    <keybind key="${kb}">${NOOP_ACTION}</keybind>`)
    .join('\n');
