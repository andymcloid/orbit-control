...
// cannot reliably preventDefault these: Alt+F4 kills the chromium window
// outright (long-press Enter on the MK118 remote), and Alt+Tab triggers
// labwc's own window cycling (long-press Tab). We map them to no-ops in
// labwc's rc.xml — see buildLabwcConfig + /api/labwc-config.
//
// Keys we deliberately do NOT suppress: Escape (harmless under --kiosk) and
// 'b' (plain letter; only matters if a page wants it). Keeping the list
// minimal avoids swallowing keystrokes a presentation legitimately needs.
...
const DEFAULT_SUPPRESS_KEYS = ['alt+f4', 'alt+tab'];

// Generate a labwc rc.xml whose only job is to neutralise the suppressed
// keystrokes. labwc resolves <keyboard><keybind> before delivering the key to
// the focused surface, so binding a chord to a no-op action consumes it.
// `<action name="None"/>` is labwc's documented "do nothing, swallow it" action.
// We intentionally emit a complete rc.xml (not a fragment) because labwc reads
// exactly one rc.xml from $XDG_CONFIG_HOME/labwc/ and does not merge.
function buildLabwcConfig(suppressKeys) {
  const binds = (Array.isArray(suppressKeys) ? suppressKeys : [])
    .map((k) => SUPPRESSIBLE_KEYS[k])
    .filter(Boolean)
    .map((k) => `    <keybind key="${k.keybind}"><action name="None"/></keybind>`)
    .join('\n');
