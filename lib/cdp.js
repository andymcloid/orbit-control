...
// Inject a stylesheet on every kiosk page so the pointer stays hidden — but
// NOT on the OrbitControl admin panel itself. The Ctrl+Alt+A hotkey navigates
// chromium to http://localhost/ (see toggleAdminPanel in server.js); there the
// operator needs to see the cursor to actually click things. Hostname guard is
// reliable because the admin URL is always localhost/127.0.0.1/0.0.0.0 while a
// real kiosk URL is a remote site.
const HIDE_CURSOR_CSS = 'html,body,*{cursor:none!important;}';
const HIDE_CURSOR_JS = '(function(){' +
  'var h=location.hostname;' +
  'if(h==="localhost"||h==="127.0.0.1"||h==="0.0.0.0")return;' +
  'function inject(){if(!document.head){return setTimeout(inject,30);}' +
  'var s=document.createElement(\'style\');s.id=\'__orbit_hide_cursor\';' +
  's.textContent=' + JSON.stringify(HIDE_CURSOR_CSS) + ';document.head.appendChild(s);}' +
  'inject();})();';
  ...
  getDebuggerUrl()
    .then((url) => {
      ...
      ws.on('message', (raw) => {
        ...
        // Screencast frame: ack immediately so chromium queues the next, and
        // hand the decoded JPEG buffer to whoever wired up onScreenshotFrame.
        if (msg.method === 'Page.screencastFrame') {
          const sessionId = msg.params && msg.params.sessionId;
          if (sessionId != null) {
            send('Page.screencastFrameAck', { sessionId }).catch(() => {});
          }
          if (onScreenshotFrame && msg.params && msg.params.data) {
            try {
              const buf = Buffer.from(msg.params.data, 'base64');
              onScreenshotFrame(buf);
            } catch {}
          }
        }
      });
...
// Event-driven preview via Page.startScreencast — chromium pushes frames
// from its own paint loop instead of us polling Page.captureScreenshot.
// Big practical win: capture cost is amortised over the page's normal
// render path (it's already drawing the next frame), and chromium drops
// frames at the source if the page didn't repaint, so an idle dashboard
// uses zero CPU instead of ~10% from constant polling.
// Frame data goes out as raw Buffer to the listener so server.js can send
// it as a binary WS frame — no base64 + JSON middleman.

let previewFps = 1;
let screencasting = false;
let screencastStarting = false;

// Preview resolution sent to clients. The actual kiosk runs at whatever
// settings.resolution says; THIS is just the screencast feed. Chromium
// downscales server-side (cheaper than scaling later), the JPEGs go raw
// over the MJPEG HTTP pipe (no encoder step), and the client's <img>
// element upscales via CSS object-fit for viewports larger than 720p.
// Mouse coords arrive as normalized 0..1 and get rescaled to the real
// kiosk viewport server-side, so preview-res is independent of kiosk-res.
// 720p quality 85 is the sweet spot: text in dashboards stays crisp, JPEG
// blockiness is mostly gone, and the LAN/tunnel handles the ~5-10 Mbps
// MJPEG stream without trouble.
const PREVIEW_MAX_WIDTH = 1280;
const PREVIEW_MAX_HEIGHT = 720;
const PREVIEW_QUALITY = 85;

function fpsToEveryNthFrame(fps) {
  // Chromium runs the renderer at up to 60Hz. everyNthFrame=2 yields ~30 fps,
  // everyNthFrame=60 yields ~1 fps (assuming the page actually repaints).
  return Math.max(1, Math.round(60 / Math.max(1, Math.min(60, fps))));
}
...
function screencastParams() {
  return {
    ...
    everyNthFrame: fpsToEveryNthFrame(previewFps),
  };
}

async function setPreviewFps(fps) {
  const clamped = Math.max(1, Math.min(30, parseInt(fps, 10) || 1));
  if (clamped === previewFps) return;
  previewFps = clamped;
  if (screencasting) {
    // Hot-swap rate: stop + restart with new everyNthFrame.
    try { await send('Page.stopScreencast'); } catch {}
    try {
      await send('Page.startScreencast', screencastParams());
      console.log('[CDP] Screencast rate set to', previewFps, 'fps');
    } catch (e) {
      console.warn('[CDP] screencast rate change failed:', e.message);
    }
  }
}

async function startPreview() {
  if (screencasting || screencastStarting || !connected) return;
  screencastStarting = true;
  try {
    await send('Page.startScreencast', screencastParams());
    screencasting = true;
    console.log('[CDP] Started screencast at ~' + previewFps + ' fps target,', PREVIEW_MAX_WIDTH + 'x' + PREVIEW_MAX_HEIGHT);
  } catch (e) {
    console.warn('[CDP] startScreencast failed:', e.message);
  } finally {
...
async function stopPreview() {
  ...
  screencasting = false;
  try { await send('Page.stopScreencast'); } catch {}
  console.log('[CDP] Stopped screencast');
}
